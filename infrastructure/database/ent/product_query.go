...
func (pq *ProductQuery) QueryCategories() *CategoriesQuery {
...
func (pq *ProductQuery) WithCategories(opts ...func(*CategoriesQuery)) *ProductQuery {
	query := &CategoriesQuery{config: pq.config}
...
//		GroupBy(product.FieldIDCategories).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Product.Query().
//		Select(product.FieldIDCategories).
//		Scan(ctx, &v)
//
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withCategories; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "id_categories" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Categories = n
			}
		}
	}
